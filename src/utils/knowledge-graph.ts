...
export class KnowledgeGraphManager {
  ...
  getEntitiesByType(sessionId: string, type: string): Entity[] {
    const rows = this.db.prepare(
      'SELECT * FROM entities WHERE session_id = ? AND type = ? ORDER BY created_at DESC'
  ...
  // Analysis operations
  analyzeContext(sessionId: string, text: string): {
    ...
    const relationPatterns = [
      { pattern: /([a-zA-Z_][a-zA-Z0-9_]*)\s+(?:calls|invokes|uses)\s+([a-zA-Z_][a-zA-Z0-9_\.]*)/g, predicate: 'calls' },
      { pattern: /([a-zA-Z_][a-zA-Z0-9_]*)\s+(?:implements|extends)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g, predicate: 'implements' },
      { pattern: /([a-zA-Z_][a-zA-Z0-9_]*)\s+(?:imports|requires)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g, predicate: 'imports' },
      { pattern: /([a-zA-Z_][a-zA-Z0-9_]*)\s+(?:contains|has)\s+([a-zA-Z_][a-zA-Z0-9_]*)/g, predicate: 'contains' }
    ];
  ...
  pruneOrphanedEntities(sessionId: string): number {
    ...
    this.db.prepare(
      `DELETE FROM entities WHERE id IN (${ids.map(() => '?').join(',')})`
    ).run(...ids);
