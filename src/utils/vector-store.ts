...
export class VectorStore {
  ...
  async search(
    ...
    const rows = this.db.prepare(
      'SELECT id, content, embedding, metadata FROM vector_embeddings'
    ).all() as any[];
  ...
  async findRelated(
    ...
    const doc = this.db.prepare(
      'SELECT content, embedding FROM vector_embeddings WHERE id = ?'
    ...
    const rows = this.db.prepare(
      'SELECT id, content, embedding, metadata FROM vector_embeddings WHERE id != ?'
