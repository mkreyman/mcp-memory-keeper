...
export class RetentionManager {
  ...
  createPolicy(policy: Omit<RetentionPolicy, 'id'>): string {
    ...
    `).run(
      id,
      policy.name,
      policyWithId.enabled ? 1 : 0,
      JSON.stringify(policyWithId)
    );
  ...
  getRetentionStats(sessionId?: string): RetentionStats {
    ...
    // Get overall stats
    const totalStats = this.db.getDatabase().prepare(`
      SELECT 
    ...
    // Get by category
    const categoryStats = this.db.getDatabase().prepare(`
      SELECT 
    ...
    // Get by priority
    const priorityStats = this.db.getDatabase().prepare(`
      SELECT 
    ...
    const eligibleStats = this.db.getDatabase().prepare(`
      SELECT 
  ...
  private findEligibleItems(policy: RetentionPolicy): any[] {
    ...
    let items = this.db.getDatabase().prepare(query).all(...params) as any[];
  ...
  private async archiveItems(items: any[], result: RetentionResult): Promise<void> {
    ...
    for (const item of items) {
      try {
        insertStmt.run(
          item.id, item.session_id, item.key, item.value,
