...
import { MigrationManager, Migration } from '../../utils/migrations';
...
describe('Database Migration Integration Tests', () => {
  ...
  describe('Error Handling', () => {
    it('should handle non-existent migration application', async () => {
      await expect(migrationManager.applyMigration('non-existent'))
    ...
    it('should handle double application', async () => {
      ...
      await expect(migrationManager.applyMigration('10.0.0', { dryRun: false }))
    ...
    it('should handle rollback without down SQL', async () => {
      ...
      await expect(migrationManager.rollbackMigration('10.1.0', { dryRun: false }))
        .rejects.toThrow('no rollback SQL');
    });
    ...
    it('should handle rollback of non-applied migration', async () => {
      ...
      await expect(migrationManager.rollbackMigration('10.2.0', { dryRun: false }))
