...
describe('Advanced Features Integration Tests', () => {
  ...
  describe('Session Branching', () => {
    beforeEach(() => {
      ...
      const items = [
        ...
        { key: 'decision_1', value: 'Architecture decision', category: 'decision', priority: 'high' },
    ...
    it('should create shallow branch with only high priority items', () => {
      ...
      const branchItems = db.prepare('SELECT * FROM context_items WHERE session_id = ?').all(branchId) as any[];
    ...
    it('should create deep branch with all items and files', () => {
      ...
      // Copy all items
      const allItems = db.prepare('SELECT * FROM context_items WHERE session_id = ?').all(testSessionId) as any[];
      ...
      // Copy files
      const files = db.prepare('SELECT * FROM file_cache WHERE session_id = ?').all(testSessionId) as any[];
      ...
      // Verify
      const branchItems = db.prepare('SELECT * FROM context_items WHERE session_id = ?').all(branchId) as any[];
      const branchFiles = db.prepare('SELECT * FROM file_cache WHERE session_id = ?').all(branchId) as any[];
  ...
  describe('Session Merging', () => {
    ...
    beforeEach(() => {
      ...
      db.prepare('INSERT INTO context_items (id, session_id, key, value) VALUES (?, ?, ?, ?)').run(
        uuidv4(), sourceSessionId, 'unique_item', 'Only in source'
      );
      db.prepare('INSERT INTO context_items (id, session_id, key, value) VALUES (?, ?, ?, ?)').run(
        uuidv4(), sourceSessionId, 'shared_item', 'Source version'
      );
      
      // Add items to target
      db.prepare('INSERT INTO context_items (id, session_id, key, value) VALUES (?, ?, ?, ?)').run(
        uuidv4(), targetSessionId, 'shared_item', 'Target version'
      );
      db.prepare('INSERT INTO context_items (id, session_id, key, value) VALUES (?, ?, ?, ?)').run(
        uuidv4(), targetSessionId, 'target_only', 'Only in target'
      );
    });
    ...
    it('should merge with keep_current conflict resolution', () => {
      ...
      for (const item of sourceItems) {
        const existing = db.prepare('SELECT * FROM context_items WHERE session_id = ? AND key = ?')
      ...
      const finalItems = db.prepare('SELECT * FROM context_items WHERE session_id = ?').all(targetSessionId) as any[];
    ...
    it('should merge with keep_source conflict resolution', () => {
      ...
      for (const item of sourceItems) {
        ...
        if (existing) {
          db.prepare('UPDATE context_items SET value = ? WHERE session_id = ? AND key = ?')
            .run(item.value, targetSessionId, item.key);
        } else {
          db.prepare('INSERT INTO context_items (id, session_id, key, value) VALUES (?, ?, ?, ?)')
            .run(uuidv4(), targetSessionId, item.key, item.value);
        }
    ...
    it('should merge with keep_newest conflict resolution', () => {
      ...
      db.prepare('UPDATE context_items SET created_at = ? WHERE session_id = ? AND key = ?')
        .run(oldDate.toISOString(), targetSessionId, 'shared_item');
  ...
  describe('Journal Entries', () => {
    ...
    it('should retrieve journal entries by session', () => {
      ...
      const journalEntries = db.prepare('SELECT * FROM journal_entries WHERE session_id = ? ORDER BY created_at')
  ...
  describe('Timeline Generation', () => {
    ...
    it('should include journal entries in timeline', () => {
      ...
      const journals = db.prepare(
        'SELECT * FROM journal_entries WHERE session_id = ? ORDER BY created_at'
      ).all(testSessionId) as any[];
  ...
  describe('Progressive Compression', () => {
    beforeEach(() => {
      ...
      const items = [
        { key: 'old_task_1', value: 'Old task 1', created_at: oldDate.toISOString(), category: 'task' },
        { key: 'old_task_2', value: 'Old task 2', created_at: oldDate.toISOString(), category: 'task' },
        { key: 'old_decision', value: 'Old decision', created_at: oldDate.toISOString(), category: 'decision' },
        { key: 'recent_task', value: 'Recent task', created_at: new Date().toISOString(), category: 'task' },
        { key: 'preserve_me', value: 'Important decision', created_at: oldDate.toISOString(), category: 'critical' },
      ];
    ...
    it('should compress old items', () => {
      ...
      const itemsToCompress = db.prepare(
        'SELECT * FROM context_items WHERE session_id = ? AND created_at < ?'
    ...
    it('should preserve specified categories', () => {
      ...
      const itemsToCompress = db.prepare(query).all(
        testSessionId, 
  ...
  describe('Cross-Tool Integration', () => {
    ...
    it('should handle events from multiple tools', () => {
      ...
      const allEvents = db.prepare('SELECT * FROM tool_events WHERE session_id = ?').all(testSessionId) as any[];
