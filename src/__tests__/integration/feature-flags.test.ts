...
describe('Feature Flags Integration Tests', () => {
  ...
  describe('Error Handling', () => {
    it('should handle non-existent flag updates', () => {
      expect(() => featureFlagManager.updateFlag('non-existent', { enabled: true })).toThrow(
    ...
    it('should handle non-existent flag enable/disable', () => {
      ...
      expect(() => featureFlagManager.disableFlag('non-existent')).toThrow(
        'Feature flag not found'
      );
    });

    it('should validate required fields', () => {
      expect(() => featureFlagManager.createFlag({ name: 'Test' } as any)).toThrow();

      expect(() => featureFlagManager.createFlag({ key: 'test' } as any)).toThrow();
    });
  });
