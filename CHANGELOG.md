...
- **Arbitrary local file read via unvalidated `context_import` filePath** (#35)
  ...
  - Import is hardened further: the resolved path must be a regular file (directories are rejected), capped at 50 MB to avoid memory exhaustion, and the whole import runs in a single transaction so a malformed row can never leave an orphaned or partially-populated session. Per-item and session-name shape are validated; `merge` only merges when a current session actually exists (and the result is reported honestly)
  ...
  - Added an E2E security regression test suite that reproduces the issue #35 PoC (arbitrary JSON read, `/etc/passwd` byte leak, `..` traversal) and covers symlink escape, the `exports-dir`-prefix sibling boundary, empty/non-string paths, directory paths, valid-JSON-but-not-an-export, and the legitimate round trip
